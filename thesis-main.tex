...
\end{lstlisting}
\par
I servizi di \texttt{PUT} e \texttt{DELETE} vengono così rifiutati, mentre per il POST è necessario un header di autenticazione; \texttt{GET} resta abilitato normalmente per tutti. La \cref{fig:api-products-put-after} mostra l'errore di autorizzazione restituito dal server dopo aver tentato di eseguire una richiesta \texttt{PUT} su \texttt{/api/Products/9} senza autenticazione.
\par
